...
public class EndView {
	...
	public static void printProductList(List<Product> list) {
		...
		for (Product p : list) {
			...
			System.out.printf("%5s|%6s|%-11s\t|%-6d|%-15S", categoryName, p.getProductCode(), p.getProductName(),
					p.getProductPrice(), p.getProductDetail());
			System.out.println();
		}
	...
	public static void printAllOrders(List<Orders> list) {
		for (Orders order : list) {
			...
			for (OrderLine ol : orderLineList) {
				...
				System.out.println(
						"상품 코드:" + ol.getProductCode() + " 상품수량:" + ol.getOrderQty() + " 가격:" + ol.getOrderPrice());
